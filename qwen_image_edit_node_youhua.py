...
class QwenImageEditPlus:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "prompt_from_node": ("STRING", {
                    "label": "提示词 (节点输入)",
                    "forceInput": True,  # 强制作为输入连接
                    "tooltip": "可连接上一个节点的输出作为提示词，优先级高于直接输入"
                }),
                "image2": ("IMAGE", {
                    "label": "图像2 (可选)"
                }),
    ...
    def generate_images(self, model, image1, prompt, api_key, num_outputs,
                       prompt_from_node=None, image2=None, image3=None, 
                       negative_prompt="低质量", prompt_extend="true", 
                       watermark="false", region="beijing", seed=-1):
        ...
        # 确定使用哪个提示词（优先级：节点输入 > 直接输入）
        final_prompt = prompt
        if prompt_from_node is not None and prompt_from_node.strip() != "":
            final_prompt = prompt_from_node
            print(f"使用来自节点的提示词: {final_prompt}")
        else:
            print(f"使用直接输入的提示词: {final_prompt}")
...
1. 同时支持两种提示词输入方式:
   - 直接在节点的"提示词"输入框中输入
   - 从上一个节点连接提示词到"提示词 (节点输入)"端口

2. 优先级逻辑:
   - 如果连接了节点输入，则优先使用节点输入的提示词
   - 如果节点输入为空，则使用直接输入的提示词
...
4. 工作流示例:
   Example 1 (直接输入):
   Load Image 1 → QwenImageEditPlus (直接输入提示词) → Save Image
   
   Example 2 (节点连接):
   Text Generator → QwenImageEditPlus (连接节点输入) → Save Image
   Load Image 1 ↗
   Load Image 2 ↗
   Load Image 3 ↗
   
   Example 3 (混合使用):
   Text Generator → QwenImageEditPlus (既连接节点输入，也保留直接输入作为备份) → Save Image
   Load Image 1 ↗
   
注意: 节点输入的优先级高于直接输入。如果没有连接节点输入或节点输入为空，则会使用直接输入的提示词。
"""
